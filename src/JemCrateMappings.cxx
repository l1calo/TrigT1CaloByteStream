...
JemCrateMappings::JemCrateMappings()
{
  ...
  double etasNeg[s_etaBinsPerRow] = {-4.05, -3.05, -2.8, -2.55 };
  double granNeg[s_etaBinsPerRow] = { 1.7,   0.3,   0.2,  0.3  };
  double etasPos[s_etaBinsPerRow] = { 2.55,  2.8,   3.05, 4.05 };
  double granPos[s_etaBinsPerRow] = { 0.3,   0.2,   0.3,  1.7  };
  ...
  int negChans[s_channels] = { 1,-1, 2, 3, -1,-1, 6, 7,  0,36,10,11,
                               5, 9,14,15, 17,13,18,19,  4,12,22,23,
			      28,20,26,27, 21,25,30,31, 33,29,34,35,
			      37,41,38,39, -1,-1,42,43 };
  int posChans[s_channels] = { 0, 1, 2,-1,  4, 5,-1,-1,  8, 9, 3,39,
...
bool JemCrateMappings::mapping(int crate, int module, int channel,
                                          ChannelCoordinate& coord) const
{
  ...
  int chan = channel;
  int quadMod = module % s_modulesPerQuadrant;
  ...
  int etaBin = chan % s_etaBinsPerRow;
  int phiBin = chan / s_etaBinsPerRow - 1;  // allow for overlap
  ...
  if (quadMod == s_extremeNegModule) {
    ...
    double etaBase = s_etaGran * s_etaBinsPerRow
...
bool JemCrateMappings::forward(int module)
{
  int quadMod = module % s_modulesPerQuadrant;
